...
@Component({
  selector: 'app-create-farasset-popup',
  templateUrl: './create-farasset-popup.component.html',
  styleUrls: ['./create-farasset-popup.component.scss']
})
export class CreateFARAssetPopupComponent implements OnInit {
    ...
    forkJoin([url1, url2, url3, url4, url5, url6, url7, url8]).subscribe(results => {
      ...
      if (!!results[0]) {
        ...
        if(!!this.LocationList && this.LocationList.length == 1){
          this.AssetInfo.controls['Location'].setValue(this.LocationList[0]);
          this.bindSubLocation(this.LocationList[0]);
        }            
        this.getFilterPlant();
      }
      if (!!results[1]) {
      ...
      if (!!results[7]) {
        ...
        if(!!this.CostCenterData && this.CostCenterData.length == 1)
            this.AssetInfo.controls['Building'].setValue(this.CostCenterData[0]);
        this.getFilterCostCenter();
      }
    })
  }
  ...
  GetBlocksOfAssets() {
    ...
    this.groupservice.BlocksOfAssetsGetByCompanyIdUserId(this.CId, userId).subscribe(response => {
      if (response) {
        ...
        // if(!!this.AssetClassData && this.AssetClassData.length == 1)
        //     this.AssetInfo.controls['BlockOfAsset'].setValue(this.AssetClassData[0]);
    ...
    this.assetsService.GetAllCustomizationDataWithType(lisTtype)
      .subscribe(Response => {
        ...
        if (Response === "[]" || Response === "" || Response === null) {
          ...
          var ClientCustColumnname = ['CPPNumber', 'AMCComment', 'ServiceProvider', 'Remark', 'Class', 'AccountingStatus', 'Division','ExpenseAccount','CostAccount','ReserveAccount','Department','Area','Merchandise','Loc2','Loc3','Loc4','Loc5','Account_Clearing','Department_Clearing','GRNDate','InterCompany','AccumulatedDepreciation','AmortizationStartDate'];
  ...
  getheaderValues(...val) {
    ...
    if(val[0].Columnname == 'GRNDate' || val[0].Columnname == 'AmortizationStartDate'){
