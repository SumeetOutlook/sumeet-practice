...
@Component({
  selector: 'app-createasset_popup',
  templateUrl: './createasset_popup.component.html',
  styleUrls: ['./createasset_popup.component.scss']
})
export class CreateAssetPopupComponent implements OnInit {
  message: any ;
  header: any ;
  ...
  ngOnInit() {
    ...
    if(this.UploadNameBy == "NFAR"){  
    this.LisTtype = "CreateNFARAsset";
    this.GetMandatoryByFlag('NFAR');
    this.UploadTypeflag =3;
    this.GetAllPreferFieldsForNFAR();
    }
    else{
      this.LisTtype = "CreateGrnAsset";
      this.UploadTypeflag =2;
      this.GetMandatoryByFlag1('GRN');
    ...
    else if (this.UploadNameBy == 'NFAR') {
     // this.AssetInfo.controls['NFAR'].setValidators([Validators.required]);
   ...
   this.groupservice.GetMandatoryByFlag(flag).subscribe((response) => {
     ...
     this.mandatoryFields.forEach(val => {        
       if(!!this.AssetInfo.controls[val]){
         if(val == 'AssetId' || val == 'barcode' || val == 'GRNNo'|| val == 'Room' || val == 'ADL2' || val == 'ADL3' || val == 'PONumber' || val == 'InvoiceNo' || val == 'SerialNo' || val == 'ITSerialNo' || val == 'equipmentNo'){
  ...
  GetAllPreferFieldsForGRN() {
    ...
    this.groupservice.GetAllGRNAssetData().subscribe(response => {
      ...
      this.EnableFormFields1();
    })
  }
  GetAllPreferFieldsForNFAR() {
    debugger;
    this.groupservice.GetNonFARAssetData().subscribe(response => {
      debugger;
      this.GRNMandatoryFields = response;
      ...
      this.GRNMandatoryFields.forEach((element, gindex) => {
        this.CostFormControls.forEach((ele, aindex) => {
          if (element.FieldsName == ele) {
            if (element.NonFarMandatory == true) {
  ...
  GetCategoryAndLocationList() {
    ...
    this.loader.open();
    let url1 = this.companyLocationService.GetLocationListByConfiguration(this.GroupId, this.UserId, this.CompanyId, this.RegionId, 19);
    let url2 = this.companyBlockService.GetCategoryListByConfiguration(this.GroupId, this.UserId, this.CompanyId, this.RegionId, 19);
  ...
  AddGRNAssetWithAssetID() {
    ...
    var assetDataList = {
      ...
      uploadBy: this.UploadNameBy,   //"GRNNo",
    ...
    this.assetsService.GetAllCustomizationDataWithType(lisTtype)
      .subscribe(Response => {
        ...
        if (Response === "[]" || Response === "" || Response === null) {
          ...
          var ClientCustColumnname = ['CPPNumber', 'AMCComment', 'ServiceProvider', 'Remark', 'Class', 'AccountingStatus', 'Division','ExpenseAccount','CostAccount','ReserveAccount','Department','Area','Merchandise','Loc2','Loc3','Loc4','Loc5','Account_Clearing','Department_Clearing','GRNDate'];
