...
export class HardwareDiscoveryReportComponent implements OnInit {
  ...
  remarkTypes: any[] = [
    { value: "1", name: "All scanned records" },
    { value: "2", name: "All Asset register records" },
    { value: "3", name: "Changed HostName in the scanned record" },
    { value: "4", name: "Changed Motherboad in the scanned record" },
    { value: "5", name: "Changed Biosno in the scanned record" },
    { value: "6", name: "Changed HostName in the Asset register" },
    { value: "7", name: "Changed Motherboad in the Asset register" },
    { value: "8", name: "Changed Biosno in the Asset register" }
  ];
  ...
  constructor(public dialog: MatDialog, private httpService: HttpClient, public localService: LocalStoreService, public datepipe: DatePipe,
    ...
    public us:UserService,private router: Router,
    public alertService: MessageAlertService,) {

      this.header1 = this.jwtAuth.getHeaders();
  ...
  ngOnInit(): void {
    ...
    this.itamService.getNetworkInventoryFilters('DEVICECATEGORY', 1).subscribe(r => {
      if (!!r && r != null) {
        this.deviceCategoryData = r;
        this.getFilterDeviceCategory();
      }
    });
    this.itamService.getNetworkInventoryFilters('CATEGORY', 1).subscribe(r => {
      if (!!r && r != null) {
        this.ListOfCategory = r;
        this.ListOfCategory1 = r;
        this.getFilterCategoryType();
      }
    });
    this.itamService.getNetworkInventoryFilters('ASSETTYPE', 1).subscribe(r => {
      if (!!r && r != null) {
        this.ListOfAssetType = r;
        this.getFilterAssetType();
      }
    });
    this.itamService.getNetworkInventoryFilters('ASSETSUBTYPE', 1).subscribe(r => {
      if (!!r && r != null) {
        this.ListOfAssetSubType = r;
  ...
  HideAndAddColumnList() {
    ...
    if (this.typeMultiCtrl == 'Company Assets') {
      ...
      this.displayedColumns = ["Icon", "MappedBy", "InventoryNo", "AssetId", "SubAssetId", "ITSerialNo", "ADL2", "ADL3", "LocationName", "CategoryName", "UserEmailID", "HostName", "MotherBoardSerialNo", "scandate", "User", "City", "GeoLocation", "IPAddress", "Gateway", "Domain", "OSType", "Manufacturer", "Model", "Processor", "NoOfCores", "HardDisk", "PhysicalMemory", "DeviceCategory", "UsedCapacityofHDD","MappedDate","MappedbyUser"];
      this.ExtraHeaderCount = 2;
      this.HardwareInfoHeaderCount = 20;
  ...
  getHardwareData(action) {
    ...
    else if (this.typeMultiCtrl == 'Company Assets' || this.typeMultiCtrl == 'Review Asset') {
    ...
    this.itamService.getNetworkInventoryAssets(data).subscribe(response => {
      ...
      if (action === 'IsExport') {
        ...
        if (!!this.typeMultiCtrl && (this.typeMultiCtrl == "Company Assets" || this.typeMultiCtrl == "Review Asset"))
  ...
  openPopUp(data: any = {}) {
    ...
    var payload = {
      ...
      Pagename : "Hardware Discovery Report"
    }
