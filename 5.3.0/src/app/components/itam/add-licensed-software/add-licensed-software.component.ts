...
@Component({
  selector: 'app-add-licensed-software',
  templateUrl: './add-licensed-software.component.html',
  styleUrls: ['./add-licensed-software.component.scss']
})
export class AddLicensedSoftwareComponent implements OnInit {
  ...
  lUsers: any[] = ['Vishal','Falgun'];
  rUsers: any[] = ['Mukesh','Hemang'];
  today = new Date();
  ...
  displayedColumns = ['License Name','Software','License Type','License Option','Installations Allowed','License Key','Cost']
  ...
  addAnotherPhoto(){
    if(this.photos.length>=3){
      this.toastr.warning("You can't upload more than 3 photos", this.message.AssetrakSays);
      return;
    } 
  ...
  moveUserLeft(){
    if(this.rUsers.length<=0 || this.rUser == '') return;
    this.lUsers.push(this.rUser);
    let idx = this.rUsers.indexOf(this.rUser);
    this.rUsers.splice(idx,1);
    this.rUser = '';
  }

  moveUserRight(){
    if(this.lUsers.length<=0 || this.lUser == '') return;
    this.rUsers.push(this.lUser);
    let idx = this.lUsers.indexOf(this.lUser);
    this.lUsers.splice(idx,1);
    this.lUser = '';
  }

  moveToSoftwareLicensePage(){
