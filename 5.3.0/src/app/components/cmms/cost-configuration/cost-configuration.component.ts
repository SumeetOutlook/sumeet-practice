...
export class CostConfigurationComponent implements OnInit {
  ...
  GetInitiatedData() {
    ...
    forkJoin([url1, url2, url3, url4, url5, url6]).subscribe(results => {
      ...
      if (!!results[2]) {
        ...
        this.displayedColumns.push('Config Type','Amount From', 'Amount To', 'Value','Levels','Mapping');
  ...
  GetAssetForTransfserBindData(Action) {
    this.loader.open();
   let data = [{'Config Type':'Amount From','Amount From':500, 'Amount To': 650, 'Value':null,'Levels':3},
   {'Config Type':'Amount From','Amount From':500, 'Amount To': 650, 'Value':null,'Levels':3},
   {'Config Type':'Amount From','Amount From':650, 'Amount To': 900, 'Value':null,'Levels':1},
   {'Config Type':'Amount Above','Amount From':null, 'Amount To': null, 'Value':900,'Levels':5}
  ];
  this.tableData = data;
  setTimeout(()=>{
  ...
  openInfoDialog(todo: String,data: any,idx:any){
    ...
    if(todo == 'view' || todo == 'add' || todo == 'edit'){
        const dialogRef = this.dialog.open(CostConfigDialogComponent, dialogData)
        dialogRef.afterClosed().subscribe(result => {
          if (!!result) { 
            ...
            if(todo=='add')  {
              ...
              this.onChangeDataSource(this.tableData);
              this.toastr.success('Part Created successfully!!',this.message.AssetrakSays);
            }
            else {
              data['Config Type'] = result['Config Type'];
              data['Amount To'] = result['Amount To'];
              data['Amount From'] = result['Amount From'];
              data['Levels'] = result['Levels'];
              data['Value'] = result['Value'];
              this.toastr.success('Part Edited successfully!!',this.message.AssetrakSays);
            }
