...
@Component({
  selector: 'app-workorder-ticket-approval',
  templateUrl: './workorder-ticket-approval.component.html',
  styleUrls: ['./workorder-ticket-approval.component.scss']
})
export class WorkorderTicketApprovalComponent implements OnInit {
  ...
  selectedAsset:any;
  displayedColumns: any[] = ['Select','Actions','ImageUrl','OrderNumber','IssueType','LocationName','AssetCriticality','StatusName','CriticalityType']
...
selectAllcheckBoxes(){
  ...
  this.datasource.data.forEach(data=>{
    this.selectedIDS.push(data.WorkOrderID);
  })
...
selectRow(element){
  let idx = this.selectedIDS.indexOf(element.WorkOrderID)
  if(idx<0)
  this.selectedIDS.push(element.WorkOrderID);
  else
