...
export class PartsPageComponent implements OnInit {
    ...
    forkJoin([url1, url2, url3, url4, url5, url6]).subscribe(results => {
      ...
      if (!!results[2]) {
        ...
        this.displayedColumns = this.ListOfField.map(choice => choice.FieldName);
           this.displayedColumns.push('OrderType','PartName','ImageURL','Mapping');
  ...
  GetAssetForTransfserBindData(Action) {
    ...
    debugger;
    this.cmmsService.GetPartsByFilter(data).subscribe(issueTypes=>{
  ...
  openInfoDialog(todo: String,data: any,idx:any){
    ...
    if(todo == 'view' || todo == 'add' || todo == 'edit'){
      this.cmmsService.GetPartByID(data.PartID).subscribe(res=>{
        debugger;
        dialogData.data.rowData = res.Model;
        const dialogRef = this.dialog.open(PartsDialogComponent, dialogData)
        dialogRef.afterClosed().subscribe(result => {
          if (!!result) { 
            ...
            if(todo=='add')  {
              ...
              data['Part Name'] = result['Part Name'];
              data['Qty'] = result['Qty'];
