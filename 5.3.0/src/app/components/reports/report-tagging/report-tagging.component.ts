...
@Component({
  selector: 'app-report-tagging',
  templateUrl: './report-tagging.component.html',
  styleUrls: ['./report-tagging.component.scss']
})
export class ReportTaggingComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  DisplayHeaders: string[] = [this.header.InventoryNumber, this.header.PrefarId, this.header.AID, this.header.SAID, this.header.AcquisitionDate, this.header.AssetClass, this.header.ADL1, this.header.ADL2, this.header.ADL3, this.header.Supplier, this.header.Location, this.header.Quantity, this.header.CostCenter, this.header.Room, this.header.StorageLocation, this.header.UserEmail, this.header.User, this.header.Custodian, this.header.AssetClassOwner, this.header.Cost, this.header.WDV, this.header.EqipmentNumber, this.header.AssetCondition, this.header.AssetCriticality, this.header.InventoryIndicator, this.header.LabelSize, this.header.LabelMaterial, this.header.Stage, this.header.ApprovedBy, this.header.ApprovedOn, this.header.InventoryOn, this.header.InventoryBy, this.header.InventoryComment, this.header.InventoryMode, this.header.ThirdPartyName, this.header.VendorLocation, this.header.VendorSentDate, this.header.CheckedOutAsset, this.header.SplitType, this.header.InventoryNote, this.header.LabelQuality, this.header.GeoCity, this.header.GeoLocation, this.header.NotFoundNote, this.header.WDVDate, this.header.TransferType, this.header.SerialNo, this.header.UOM, this.header.AssetExpiryDate, this.header.ITSerialNo, this.header.SBU, this.header.PONumber, this.header.InvoiceNumber, this.header.AllocationType, this.header.InsuranceTo, this.header.AMCExpiryDate, this.header.InstallationDate, this.header.WarrantyExpiryDate, this.header.MachineRegdNo, this.header.TypeOfAsset, this.header.SubTypeOfAsset, this.header.Document, this.header.PreviousInventoryInfo, this.header.VerifiedAt, this.header.ADIDate, this.header.Photo];
  ...
  SBUGetdata() {
   ...
   // this.userRoleService.GetAllSBU(companyId).subscribe(r => {
    this.cls.GetLocationListByConfiguration(this.SessionGroupId, this.SessionUserId, this.SessionCompanyId, this.SessionRegionId, 56).subscribe(r => {
 ...
 openPopUp(data: any = {}) {
   ...
   this.ListOfField.forEach((element,index)=>{
    if(element.Custom1 == 'Photo' || element.Custom1 =='Document' ) 
  ...
  getTaggingData(action) {
    ...
    if( this.ReportForm.value.periodMultiCtrl.length == 0){
    ...
    this.reportService.GETASSETREGISTERREPORTBYSP(parameters).subscribe(response => {
      ...
      if (action === 'IsExport') {
        ...
        // this.paginationParams.totalCount =  0;
        this.onChangeDataSourceC(this.resultData.taggingcomplete);
      }
    });
  }
  ...
  GetInitiatedData() {
debugger;
    let url3 = this.groupservice.GetFieldListByPageId(56,this.SessionUserId,this.SessionCompanyId);
  //  let url6 = this.groupservice.GetFieldList(56,this.SessionUserId);
  ...
  GetPhoto(currentdata) {
    if (!!currentdata) {
      ...
      this.reportService.GetPhotoForReport(prefarId, documentType).subscribe(response => {
        ...
        var path = this.SessionCompanyId + "/Photo/" + response;
