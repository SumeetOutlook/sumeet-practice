...
displayTable: boolean = false;
  ...
  toDateValidation() {
    debugger;
    if(this.ReportForm.controls['fromDate'].value)   
      this.FromDate = new Date(this.ReportForm.controls['fromDate'].value);
  }
...
this.ReportForm.value.toDate = moment(this.ReportForm.value.toDate).format('DD-MMM-YYYY');
    var data =
    {
      ...
      option: this.ReportForm.value.optionMultiCtrl,//optionMultiCtrl
      FromDate:this.ReportForm.value.fromDate,//this.ReportForm.value.fromDate,
  ...
  GetInitiatedData() {
    ...
    let url3 = this.groupservice.GetFieldListByPageId(109,this.SessionUserId,this.SessionCompanyId);
